...
By default a evals run will run all programming exercises in [O Code Evals](https://github.com/RooCodeInc/Roo-Code-Evals) repository with the Claude Sonnet 4 model and default settings. For basic configuration you can specify the LLM to use and any subset of the exercises you'd like. For advanced configuration you can import a O Code settings file which will allow you to run the evals with O Code configured any way you'd like (this includes custom modes, a footgun prompt, etc).
...
- Optionally builds and installs the O Code extension from source
