	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			if (key === "chat:announcement.title") {
				return `🎉 O Code ${options?.version} Released`
			}
			if (key === "chat:announcement.description") {
				return `O Code ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(screen.getByText(`🎉 O Code ${expectedVersion} Released`)).toBeInTheDocument()
		expect(
			screen.getByText(
				`O Code ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
